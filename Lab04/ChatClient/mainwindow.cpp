...
{
    m_chatClient->connectToServer(QHostAddress(ui->serverAddressEdit->text()),1967);
}
...
void MainWindow::on_sendButton_clicked()
{
    if(!ui->inputLineEdit->text().isEmpty())
        m_chatClient->sendMessage(ui->inputLineEdit->text());
}
...
void MainWindow::messageReceived(const QString &sender, const QString &text)
{
    ...
    qDebug() << "Message received from" << sender << ":" << text;  // 添加日志以调试
}
...
void MainWindow::jsonReceived(const QJsonObject &docObj)
{
    ...
    if (typeVal.toString().compare("message", Qt::CaseInsensitive) == 0) {
        ...
        qDebug() << "Message received from" << senderVal.toString() << ":" << textVal.toString();  // 添加日志以调试
        messageReceived(senderVal.toString(), textVal.toString());  // 确保这里正确显示消息
    }
...
{
    qDebug() << "User list received:" << list;
